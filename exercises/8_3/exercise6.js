...
function studentAverage() {
  return notas.map( (element, index) => {
    const averg = element.reduce( (acc, current) => (acc + current), 0)/element.length;
    return { 'name': alunos[index], 'average': averg }
  });
}
 ...
 * aqui além de reduce será necessário utilizar também a função map. Dica: 
 * Você pode acessar o index do array dentro de map, e você pode ver o 
